...
description: Orchestrate parallel coding tasks across `worktrunk` worktrees with Orca. Triggers for spawn, dispatch, poll, or `/review-gate` merged results.
---
...
- **Coordinate from the main worktree.** Create the orchestration run and dispatch workers from the main worktree (the original clone containing the real `.git/` directory), never from a linked worktree created by `git worktree add`. If dispatch fails with `consumer_fenced`, rebind with `orca orchestration run-use --id <run-id>`.
