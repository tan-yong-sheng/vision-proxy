...
- `includeContext` contradiction FLAGGED: `docs/CONFIG.md:46` says default `false`, but `src/core.ts:345` `DEFAULT_CONFIG.includeContext=true`. Both reports repeat the doc value without noting the mismatch. Also VERIFIED `includeContext` has zero consumers outside `src/core.ts` (grep: only `core.ts:43,345,383,560,598,639,691`; `readEnvOverrides` parses `VP_INCLUDE_CONTEXT` but nothing reads `config.includeContext` in pipeline/adapter/hooks). So gating proposal must wire it fresh; resolve the default-truth conflict before documenting. Confidence 100% on the gap.
...
### Step 0 — Resolve open decisions (5 min)
- Default truth: `core.ts:true` vs `CONFIG.md:false` for `includeContext` — pick one, fix the other file in same PR.
...
- `pipeline.ts`: `const context = flags.context ?? ""`; pass `context` into both `analyzeImpl()` calls (single line ~174, joint line ~219); extend cache key: `hashImageData(question + "\n" + context)` or add 5th param to `buildToolCacheKey` — must bump key so question-only entries do not collide with question+context entries.
...
- **Pi `src/pi-extension.ts`**: closure-scoped `recentUserTexts: string[]`; `context` handler slices last-8 user texts from `event.messages` via shared parser (handles string + TextContent[]), updates stash atomically; `tool_result` (read+image) joins stash with `\n\n---\n\n`, passes as `contextText` third arg to extended `runAnalyze(images, signal?, contextText?)` → `buildAnalyzeArgs(..., undefined, context)`. Missing/empty stash → omit `--context`.
- **opencode `src/opencode-plugin.ts`**: module `Map<sessionID, {role,text}[]>` ring buffer (cap 8/session); `handleChatMessage` pushes current user text (after INJECTION_MARKER strip), builds context from others; `handleToolExecuteBefore` (read+image) reads buffer → `runAnalyze([file], context)` → `buildAnalyzeArgs(..., undefined, context)`. Evict on session end / bound map size. No SDK polling on hot path.
